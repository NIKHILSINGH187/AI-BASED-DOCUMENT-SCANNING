  ...
  return (
    <div className="relative space-y-6">
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute -top-20 left-1/3 h-[400px] w-[400px] animate-glow-pulse rounded-full bg-cyan-500/[0.05] blur-[120px]" />
        <div className="absolute bottom-0 right-0 h-[350px] w-[350px] animate-float-slow rounded-full bg-blue-500/[0.04] blur-[100px]" />
      </div>
      ...
      <div className="flex animate-fade-in-up items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Identity verification overview and analytics</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="glass-panel animate-fade-in-up p-5 lg:col-span-2" style={{ animationDelay: '150ms' }}>
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-600">Verification Volume</h2>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={volumeData} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: 'rgba(255,255,255,0.95)', border: '1px solid #e2e8f0', borderRadius: '10px', fontSize: '12px', boxShadow: '0 4px 20px rgba(15,23,42,0.1)' }}
        ...
        <div className="glass-panel animate-fade-in-up p-5" style={{ animationDelay: '250ms' }}>
          <h2 className="text-sm font-semibold text-slate-600">Risk Distribution</h2>
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="glass-panel animate-fade-in-up p-5" style={{ animationDelay: '350ms' }}>
          <h2 className="text-sm font-semibold text-slate-600">Document Types</h2>
        ...
        <div className="glass-panel animate-fade-in-up p-5 lg:col-span-2" style={{ animationDelay: '450ms' }}>
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-600">Recent Cases</h2>
            <button
              onClick={() => navigate('/cases')}
              className="group flex items-center gap-1 text-xs text-cyan-600 transition-colors hover:text-cyan-700"
            >
                ...
                {loading ? (
                  ...
                  recentCases.map((c, i) => (
                    <tr
                      ...
                      className="animate-fade-in-up cursor-pointer border-b border-slate-100 transition-colors hover:bg-slate-50"
                      style={{ animationDelay: `${i * 60}ms` }}
                    >
                      <td className="px-3 py-3 font-mono text-xs text-cyan-600">{c.case_id}</td>
                      <td className="px-3 py-3 text-slate-700">{c.document_type}</td>
